...
for n=1:nargin
   ...
   elseif(isa(varargin{n},'RandStream'))
      ...
      error(message('comm:awgn_R:InvalidArg'));
   end
end
...
switch sigStr
   ...
   % --- awgn_R(x, snr, sigPower)
   case 'n/n/n'
      sig      = varargin{1};
      reqSNR   = varargin{2};
      sigPower = varargin{3};

   % --- awgn_R(x, snr, 'measured')
   case 'n/n/s'
      sig      = varargin{1};
      reqSNR   = varargin{2};
   ...
   % --- awgn_R(x, snr, sigPower, 'db|linear')
   case 'n/n/n/s'
      sig      = varargin{1};
      reqSNR   = varargin{2};
      sigPower = varargin{3};
   ...
   % --- awgn_R(x, snr, 'measured', 'db|linear')
   case 'n/n/s/s'
      sig      = varargin{1};
      reqSNR   = varargin{2};
      measMode = lower(varargin{3});
   ...
   % --- awgn_R(x, snr, sigPower, state, 'db|linear')
   ...
   % --- awgn_R(x, snr, 'measured', state, 'db|linear')
   ...
   otherwise
      error(message('comm:awgn_R:InvalidSyntax'));
end   
...
% --- sig
if(isempty(sig))
   error(message('comm:awgn_R:NoInput'));
end

if(ndims(sig)>2)
   error(message('comm:awgn_R:InvalidSignalDims'));
end

% --- measMode
if(measModeSet)
   if(~strcmp(measMode,'measured'))
      error(message('comm:awgn_R:InvalidSigPower1'));
   end
end
...
if(pModeSet)
   ...
   case {'db' 'linear'}
   otherwise
      error(message('comm:awgn_R:InvalidPowerType'));
   end
end

% -- reqSNR
if(any([~isreal(reqSNR) (length(reqSNR)>1) (isempty(reqSNR))]))
   error(message('comm:awgn_R:InvalidSNR'));
end

if(strcmp(pMode,'linear'))
   if(reqSNR<=0)
      error(message('comm:awgn_R:InvalidSNRForLinearMode'));
   end
end
...
if(~strcmp(measMode,'measured'))
   ...
   if(any([~isreal(sigPower) (length(sigPower)>1) (isempty(sigPower))]))
      error(message('comm:awgn_R:InvalidSigPower2'));
   end
   
   if(strcmp(pMode,'linear'))
      if(sigPower<0)
         error(message('comm:awgn_R:InvalidSigPowerForLinearMode'));
      end
   end
...
y = sig+whitenoise(size(sig,1), size(sig,2), noisePower, 1, state, pMode, opType);
